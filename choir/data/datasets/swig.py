...
__all__ = ["register_swig_instances, load_swig_json"]

logger = logging.getLogger(__name__)


def load_swig_json(json_file, image_root, dataset_name=None):
    """
    ...
    for anno_dict in imgs_anns:
        ...
        for obj in anno_dict["box_annotations"]:
            ...
            if len(aux_categories) == 0:
                aux_classes.append(-1)
            else:
                aux_classes.append(id_map[aux_categories[0]])
                ...
                obj["interaction"].append(
                    {
                        ...
                        "action_id": actions,
                        "bbox_mode": BoxMode.XYWH_ABS,
                        "aux_category_id": aux_classes[object_id],
                    }
                )
...
def register_swig_instances(name, metadata, json_file, image_root):
    """
    Register a swig dataset in json annotation format for human-object
    interaction detection (i.e., `swig_*.json` in the dataset).
    ...
    DatasetCatalog.register(name, lambda: load_swig_json(json_file, image_root, name))
