...
class HOIRepeatFactorTrainingSampler(Sampler):
    """
    Similar to TrainingSampler, but a sample may appear more times than others based
    on its "repeat factor". This is suitable for training on class imbalanced datasets like LVIS.
    """
    ...
    def repeat_factors_from_category_frequency(dataset_dicts, repeat_thresh):
        ...
        for dataset_dict in dataset_dicts:  # For each image (without repeats)
            ...
            for ann in dataset_dict["annotations"]:
                for hoi in ann["interaction"]:
                    for action_id in np.where(hoi["action_id"])[0]:
                        cats.add("{}_{}".format(action_id, hoi["category_id"]))
            for cat_id in cats:
