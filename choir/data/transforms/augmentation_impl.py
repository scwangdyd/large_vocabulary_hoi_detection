...
__all__ = [
    ...
    "RandomCrop_InteractionConstraint",
]
...
class RandomCrop_InteractionConstraint(Augmentation):
    ...
    def get_transform(self, image, boxes):
        h, w = image.shape[:2]
        croph, cropw = self.crop_aug.get_crop_size((h, w))
        assert h >= croph and w >= cropw, "Shape computation in {} has bugs.".format(self)
        h_choice = np.arange(0, h - croph + 1)
        w_choice = np.arange(0, w - cropw + 1)
        h_prob, w_prob = np.ones_like(h_choice), np.ones_like(w_choice)
        for box in boxes:
