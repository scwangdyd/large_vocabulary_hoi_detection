...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        qkv_bias (bool, optional):  If True (default), add a learnable bias to query, key, value.
    ...
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter( # 2*Wh-1 * 2*Ww-1, nH
...
class SwinTransformer(Backbone):
    ...
    def output_shape(self):
        return {i: ShapeSpec(channels=self.num_features[i], stride=2 ** (i+2))
...
def build_swin_transformer_backbone(cfg, input_shape):
    ...
    ape               = cfg.MODEL.SWIN.ABSOLUTE_POSITION_EMBED
    out_features      = cfg.MODEL.SWIN.OUT_FEATURES
    # fmt: on
    
    return SwinTransformer(pretrain_img_size=pretrain_img_size,
                           ...
                           ape=ape,
                           patch_norm=True,
                           out_indices=out_features,#(0, 1, 2, 3),
                           frozen_stages=-1,
                           use_checkpoint=False)
