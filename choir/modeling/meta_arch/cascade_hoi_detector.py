...
class CHOIR(nn.Module):
    """
    Cascade Human-Object Interaction Detection with Transformers (CHOIR).
    ...
    def forward(self, batched_inputs):
        ...
        masks = self.interpolate_mask(masks, {f: x.shape for f, x in features.items()})
    ...
    def inference(self, batched_inputs, do_postprocess=True):
        ...
        if do_postprocess:
            return CHOIR._postprocess(results, batched_inputs, images.image_sizes)
        else:
            return results
    ...
    def interpolate_mask(self, masks, features_shape):
        """
        Interpolate the masks based on the size of features from the backbone.
        """
        return_masks = {}
        for f, shape in features_shape.items():
            masks_f = F.interpolate(
                masks[:, None, ...].float(), size=shape[-2:]
            ).to(torch.bool)[:, 0, ...]
            return_masks[f] = masks_f
        return return_masks
