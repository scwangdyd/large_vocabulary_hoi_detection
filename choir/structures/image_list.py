    ...
    def from_tensors(
        ...
        if len(tensors) == 1:
            ...
            if all(x == 0 for x in padding_size):  # https://github.com/pytorch/pytorch/issues/31734
                ...
                masks = torch.zeros(image_size, dtype=torch.bool, device=tensors[0].device)
                masks = masks.unsqueeze(0)
            else:
                padded = F.pad(tensors[0], padding_size, value=pad_value)
                batched_imgs = padded.unsqueeze_(0)
                masks = torch.zeros(image_size, dtype=torch.bool, device=tensors[0].device)
                masks = F.pad(masks, padding_size, value=1).unsqueeze_(0)
        else:
            ...
            # mask to specify padding elements which will be ignored by the transformers
            batch_shape = (len(tensors),) + tuple(max_size[1:]) # ignore the color channels
            masks = torch.ones(batch_shape, dtype=torch.bool, device=tensors[0].device)
            for img, m in zip(tensors, masks):
                m[..., : img.shape[-2], : img.shape[-1]] = False

        return ImageList(batched_imgs.contiguous(), image_sizes), masks
