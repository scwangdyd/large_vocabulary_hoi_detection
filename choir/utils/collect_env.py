...
def collect_env_info():
    ...
    try:
        ...
        data.append(
            ("choir", choir.__version__ +
             " @" + os.path.dirname(choir.__file__))
        )
    except ImportError:
        data.append(("choir", "failed to import"))

    try:
        from choir import _C
    except ImportError:
        data.append(("choir._C", "failed to import. choir is not built correctly"))
        ...
        if has_cuda:
            data.append(
                ("choir arch flags", detect_compute_compatibility(CUDA_HOME, _C.__file__))
            )
