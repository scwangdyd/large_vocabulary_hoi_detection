...
class Visualizer:
    ...
    def __init__(self, img_rgb, metadata=None, scale=1.0, instance_mode=ColorMode.IMAGE, fontsize=16):
        """
        Args:
        ...
        self._default_font_size = max(
            np.sqrt(self.output.height * self.output.width) // 90, fontsize // scale # 10 // scale
        )
    ...
    def draw_dataset_dict(self, dic):
        """
        Draw annotations/segmentaions in choir Dataset format.

        Args:
            dic (dict): annotation/segmentation data of one image, in choir Dataset format.
        ...
        *,
        ...
        Args:
            ...
            masks (masks-like object): Supported types are:
                ...
                * :class:`choir.structures.PolygonMasks`,
                  :class:`choir.structures.BitMasks`.
...
class InteractionVisualizer(Visualizer):
    def __init__(
        ...
        # too small texts are useless, therefore clamp to 9
        self._default_font_size = max(
            np.sqrt(self.output.height * self.output.width) // 90, footsize // scale
        )
        self._instance_mode = instance_mode

    def draw_interaction_predictions(self, interaction_boxes, labels):
        """
        ...
        Returns:
            output (VisImage): image object with visualizations.
        """
        num_instances = len(interaction_boxes)
        if num_instances == 0:
            return self.output 
        
        person_boxes = interaction_boxes[:,:4]#self._convert_boxes(predictions.person_boxes)
        object_boxes = interaction_boxes[:,4:8]
        ...
        labels_to_draw = {tuple(x): [] for x in unique_object_boxes}
        for i in range(num_instances):
            x = tuple(person_boxes[i])
            interactions_to_draw[x].append(object_boxes[i])
            if labels is not None:
                labels_to_draw[tuple(object_boxes[i])].append(
                    {
                        "label": labels[i],
    ...
    def overlay_interactions(
        ...
        *,
        ...
        if assigned_person_colors is None:
            assigned_person_colors = ["b"] * len(unique_person_boxes)
        if assigned_object_colors is None:
            assigned_object_colors = ["g"] * len(unique_object_boxes)
        ...
        for person_box, object_list in interactions.items():
            for object_box in object_list:
                ...
                self.draw_line([p_xc, o_xc], [p_yc, o_yc], color="w", linewidth=linewidth)
